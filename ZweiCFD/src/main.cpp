    ...
    while (!WindowShouldClose()) {
        ...
        if (gamma.size() > 0) {
            ...
            for (size_t i = 0; i < panels.size() - 1; ++i) {
                ...
                Vector2 nodePos2 = { (float)(panels[i+1].p1.x * renderScale), (float)(-panels[i+1].p1.y * renderScale)};
                float gVal2 = static_cast<float>(gamma(i+1));
                Vector2 gEnd2 = {
                    nodePos2.x + (float)(panels[i+1].normal.x * gVal2 * gammaDisplayScale),
                    nodePos2.y + (float)(-panels[i+1].normal.y * gVal2 * gammaDisplayScale)
                };
                
                Color gColor = (gVal1 >= 0.0f) ? RED : BLUE;
                
                DrawLineEx(nodePos1, gEnd1, 1.0f, Fade(gColor, 0.2f)); 
                DrawLineEx(gEnd1, gEnd2, 2.0f, gColor); 
                DrawCircleV(gEnd1, 1.5f, gColor);
            }

            size_t lastNode = panels.size() - 1;
            Vector2 lastPos = { (float)(panels[lastNode].p2.x * renderScale), (float)(-panels[lastNode].p2.y * renderScale)};
            float gValLast = static_cast<float>(gamma(panels.size()));
            Vector2 gEndLast = {
            ...
            DrawLineEx(lastPos, gEndLast, 1.0f, Fade(gColorLast, 0.2f));
            DrawCircleV(gEndLast, 1.5f, gColorLast);
        }
